    ...
    <div className={`w-full ${vertical ? 'max-w-none' : 'max-w-2xl'} mx-auto max-h-full bg-white dark:bg-[#111827] border border-slate-200 dark:border-white/10 rounded-4xl shadow-[0_24px_90px_rgba(15,23,42,0.12)] dark:shadow-[0_24px_90px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col relative select-none transition-all ${compact ? 'scale-[0.98]' : ''}`}>
      ...
      <div className={`${pad} flex items-start justify-between gap-4 border-b border-slate-100 dark:border-white/10`}>
        <div className="flex items-start gap-3.5 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 min-w-0">
              <h3 className={`font-black text-slate-900 dark:text-slate-50 truncate ${vertical ? 'text-lg' : 'text-base'}`}>
                {job.companyName}
              </h3>
              {job.companySize && (
            ...
            <h2 className={`font-black text-indigo-700 dark:text-indigo-300 leading-snug mt-0.5 line-clamp-2 ${vertical ? 'text-xl sm:text-2xl lg:text-[26px]' : 'text-lg'}`}>
              {job.role}
            </h2>
          </div>
        </div>
        ...
        <div className="flex flex-col items-end flex-none">
          <div className="flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-500/15 border border-emerald-200 dark:border-emerald-500/30 text-emerald-800 dark:text-emerald-300 font-black text-sm shadow-sm">
            <Sparkles className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
            <span>{job.staticMatchScore}%</span>
          </div>
          <span className="text-[10px] text-slate-500 dark:text-slate-500 font-bold mt-1 uppercase tracking-wider">Match</span>
        </div>
      </div>
      ...
      <div className={`${pad} flex-1 min-h-0 overflow-y-auto space-y-4 [scrollbar-width:thin] [scrollbar-color:rgb(226_232_240)_transparent]`}>
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="flex items-center gap-2 p-2.5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 text-slate-800 dark:text-slate-100">
            ...
            <div className="truncate">
              ...
              <p className="font-extrabold text-slate-900 dark:text-slate-50 truncate">{job.location}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 p-2.5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 text-slate-800 dark:text-slate-100">
            ...
            <div className="truncate">
              ...
              <p className="font-extrabold text-slate-900 dark:text-slate-50 truncate">{postedLabel}</p>
            </div>
          </div>
        </div>

        <p className={`text-sm text-slate-700 dark:text-slate-200 leading-relaxed font-medium ${vertical ? 'line-clamp-5' : 'line-clamp-4'}`}>
          {snippet}
        </p>
        ...
        {job.missingSkills && job.missingSkills.length > 0 && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 dark:border-amber-500/30 dark:bg-amber-500/10 px-4 py-3 text-xs font-semibold text-amber-900 dark:text-amber-200">
      ...
      <div className="px-4 pb-4">
        {hasApplyUrl ? (
          ...
          <button
            type="button"
            disabled
            className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-100 dark:bg-white/[0.05] px-4 py-3 text-xs font-black uppercase tracking-wide text-slate-400 dark:text-slate-600 border border-slate-200 dark:border-white/10 cursor-not-allowed"
          >
